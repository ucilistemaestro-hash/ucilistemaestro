...
export default function ProfesorRasporedPage() {
  ...
  async function ucitajRaspored() {
    ...
    if (error) {
      setGreska("Nije moguće učitati raspored.");
    } else {
      setRaspored(data ?? []);
    }
  ...
  function krajPredavanja(predavanje: Predavanje) {
    return new Date(
  ...
  const buducaPredavanja = raspored.filter(
    (predavanje) => krajPredavanja(predavanje) >= sada
  );
        ...
        {ucitavanje ? (
          ...
          <>
            ...
            <div className="space-y-5">
              {buducaPredavanja.map((predavanje, index) => (
                <article
